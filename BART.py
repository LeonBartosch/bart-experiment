...
REPETITIONS_PRACTICE = 10 # repetitions of practice
REWARD = 0.05
WEIGHT = 0.01
...
def get_valid_number(min_value, max_value):
    error_message = ""
    while True:
        dlg = gui.Dlg(title="Estimation")
        if error_message:
            dlg.addText(error_message, color="red")  # Zeigt Fehler in Rot
        dlg.addField(f"How many pumps do you think you can achieve before popping the balloon?")
        dlg.show()

        if dlg.OK:  # Falls OK gedrückt wurde
            try:
                user_input = int(dlg.data[0])  # Eingabe in eine Zahl umwandeln
                if min_value <= user_input <= max_value:
                    return user_input  # Falls gültig, zurückgeben
                else:
...
def bart(info):
    ...
    for trialNumber, trial in enumerate(trials):
        ...
        estimate = get_valid_number(1, max(MAX_PUMPS))
